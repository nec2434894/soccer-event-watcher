...
def passes_filters(title: str, snippet: str, url: str, keyword: str, cfg: dict) -> bool:
    ...
    # 4) 地域名 または 指定クラブ名(いずれかのキーワード)を含むこと
    region_hit = any(r in text for r in cfg["regions"])
    keyword_hit = any(k in text for k in cfg["keywords"])
    if not (region_hit or keyword_hit):
        return False
...
# ---------------------------------------------------------------- 収集
def search_google_news(keyword: str, cfg: dict) -> list:
    """Google ニュース RSS で キーワード×イベント語 を検索する。"""
    results = []
    # クエリ例: 浦和レッズ (サイン会 OR 公開練習 OR ...) -site:mercari.com ...
    event_part = " OR ".join(cfg["event_words"])
    minus_part = " ".join(f"-site:{d}" for d in cfg["exclude_domains"][:8])
    query = f'{keyword} ({event_part}) {minus_part}'
...
def search_custom_search(keyword: str, cfg: dict) -> list:
    ...
    try:
        ...
        print(f"WARN: Custom Search API 失敗 ({keyword}): {e}")
        return []
...
def collect(cfg: dict) -> dict:
    """全キーワードを検索し、フィルタ済みイベントを URL キーの辞書で返す。"""
    now_iso = datetime.now(JST).isoformat()
    found = {}

    for keyword in cfg["keywords"]:
        raw = []
        raw += search_google_news(keyword, cfg)
        raw += search_custom_search(keyword, cfg)
        ...
        for item in raw:
            if not item["url"]:
                continue
            if not passes_filters(item["title"], item["snippet"], item["url"], keyword, cfg):
                continue
            ...
            accepted += 1
        print(f"  {keyword}: 取得 {len(raw)} 件 → 採用 {accepted} 件")
...
def build_html(events: list, cfg: dict) -> str:
    ...
    keywords_label = " / ".join(html.escape(k) for k in cfg["keywords"])
